...
data_path <- "G:/Mijn Drive/Soortenmeetnetten/Oplevering data/2025/Data niet in Meetnetten.be/Hamster - Oplevering data Meetnetten 2025"

burchten_nieuw_sf <- read_sf(data_path, "Meetnet Hamster_2025_waarnemingen",
                             crs = 31370)

burchten_nieuw <- burchten_nieuw_sf %>%
  rename(id_wnm = unieke_id, soort_wet = naam_lat, soort_nl = naam_nl, opmerking = bijzonderh) %>%
...
percelen_update_sf_orig <- read_sf(data_path, "Meetnet Hamster_2025_percelen")

percelen_sf <- percelen_update_sf_orig %>%
  rename(id_perceel = id, aantal_tellers = `#tellers`, oppervlakte_m2 = `opp (m2)`) %>%
  mutate(meetnet = "Hamster",
         datum = as.Date(datum, format = "%d/%m/%Y"),
         type_telling = ifelse(datum > as.Date("2024-06-01"), "zomer", "winter"),
         oppervlakte_ha = oppervlakte_m2 / 10000) %>%
