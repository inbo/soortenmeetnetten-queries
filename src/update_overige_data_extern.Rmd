...
data_path <- "G:/Mijn Drive/Soortenmeetnetten/Oplevering data/2021/Data niet in Meetnetten.be/Hamster - Oplevering data Meetnetten 2021"

burchten_nieuw_sf <- read_sf(data_path, "Hamsterburchten 2021_meetnet_waarnemingen_Lambert72_ontdubbeld_finale versie",
                             crs = 31370)
...
percelen_update_sf_orig <- read_sf(data_path, "Hamster_percelen_onderzocht_2021_Lambert72_finale versie")

percelen_sf <- percelen_update_sf_orig %>%
  rename(id_perceel = id, aantal_tellers = X.Tellers, opmerking = burchten_1, gewas = Gewas, oppervlakte_ha = opp..ha.) %>%
  mutate(meetnet = "Hamster",
...
oppervlakte_onderzocht <- percelen_sf %>%
  ...
  summarise(oppervlakte_ha_totaal = sum(oppervlakte_ha),
            area_calc_totaal = round(sum(area_calc), 1),
