...
data_ruw_2020 <- planten_2020_orig %>%
  ...
  select(id, meetnet = naam_nl,  datum,  x, y, aantal, floroncode = Floroncode, typewaarneming, waarnemer = wnr, status, locatie_np = Locatie, ingevoerd = Meetnet, opm = bijzonderheden)%>%
...
data_waarnemingen <- data_extern_locaties %>%
...
aantallen_waarnemingen_mn <- data_waarnemingen %>%
  ...
  select(bron, meetnet, locatie, afstand_tot_locatie, jaar, datum, id_wnm = id, code = floroncode,  schaal , aantal, x =x_coord, y = y_coord, visit_id, opm)
...
write.csv2(aantallen_planten, "../processed/data_planten_2020.csv", row.names = FALSE)

write.csv2(aantallen_waarnemingen_extra, "../processed/data_planten_2020_extra.csv", row.names = FALSE)
```
