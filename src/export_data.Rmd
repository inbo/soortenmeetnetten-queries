...
aantallen_planten_puntlocaties <- aantallen_planten %>%
  ...
  select(meetnet, protocol, locatie, datum,  soort_w = soort_wet, visit_id, validatie_bezoek = validatie, track, sample_id, schaal, code, beschrijving_floroncode, opm)
...
```{r}
query_tracks_planten <- "select PG.name as soortgroep
    , P.name as meetnet
    , L.name as locatie
    , PR.i18n->>'name_nl' as protocol
    , V.id as visit_id
...
tracks_planten <- st_read(dsn = con, query = query_tracks_planten) %>%
...
st_write(aantallen_planten_puntlocaties,  "../raw/planten_tracks", layer = "planten_tracks", driver = "ESRI Shapefile", delete_layer = TRUE)
