...
aantallen <- aantallen_orig %>%
  ...
  group_by(sample_id, species_id, geslacht, levensstadium, activiteit) %>%
  mutate(rank_temp = rank(aantal, ties.method = "random"),
         n_temp = n()) %>%
  ungroup() %>%
  mutate(type_aantal = ifelse(protocol %in% c("Vlinders - Gebiedstelling (v1)", "Vlinders - Gebiedstelling"),
                              ifelse(n_temp == 1, "totaal gezien",
                                     ifelse(rank_temp == 1 & n_temp ==2, "maximum samen gezien",
                                            ifelse(rank_temp == 2, "totaal gezien", NA))),
                              "totaal gezien") 
                      ) %>%
  select(-rank_temp, -n_temp)
...
aantallen_wintertellingen <- set_utf8(dbGetQuery(con, query_aantallen_abv)) 
