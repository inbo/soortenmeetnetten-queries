...
aantallen_planten <- aantallen_planten %>%
  ...
  mutate(track = ifelse(is.na(geom_text), 0, 1))

aantallen_planten_tracks <- aantallen_planten %>%
  filter(track == 1) %>%
  select(meetnet, protocol, locatie, datum,  soort_w = soort_wet, sample_id, schaal, code, beschrijving_floroncode, opm)
...
tellers <- set_utf8(dbGetQuery(con, query_tellers))
```
...
write_vc(aantallen, "aantallen", root = "../raw", sorting = c("soortgroep", "meetnet", "locatie", "visit_id", "sample_id"), strict = FALSE)
...
write_vc(meetnetten_users, "meetnetten_users", root = "../raw", sorting = c("meetnet", "last_name"))
...
st_write(aantallen_planten_tracks, "../raw/planten_tracks.gpkg", layer = "planten_tracks", driver = "GPKG", delete_layer = TRUE)
st_write(aantallen_planten_tracks,  "../raw/planten_tracks", layer = "planten_tracks", driver = "ESRI Shapefile", delete_layer = TRUE)
```
