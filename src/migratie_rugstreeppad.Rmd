...
source(file.path(path, "source/functions_smp.R"))
```
...
visits_meetnetten <- get_visits_smp() %>%
  filter(meetnet == "Rugstreeppad") %>%
  select(visit_id, hoofdteller, bezoek_status, opmerking)
...
data_2022_migratie <- data_rugstreeppad_2022 %>%
  ...
  mutate(meetnet = "Rugstreeppad",
         ...
         bezoek_status = "Conform protocol",
         zoekinspanning = "enkel eisnoeren",
         hoofdteller = "Tim Vochten") %>%
  select(meetnet, protocol, locatie, datum, hoofdteller , bezoek_status, zoekinspanning, levensstadium = kleed, aantal)
...
aantallen_migratie_2021_update <- aantallen_migratie_2021_update %>%
  ...
  mutate(wijziging_sample = ifelse(wijziging == "bezoek opsplitsen",
                        ...
                        NA)) %>%
  mutate(bezoek_status = "Conform protocol",
         zoekinspanning = "Enkel eisnoeren") %>%
