...
locations_year <- visits_location %>%
  ...
  summarise(n_visits_tot = sum(n_visits) ,
            n_locaties = n_distinct(visit_location),
            n_locaties_5bezoeken = sum(n_visits >= 5),
            n_locaties_5bezoeken_data = sum(n_visits_data >= 5)) %>%
  ungroup()

write_csv2(data_stagbeetle, "../output/stag_beetle_2024-01-08.csv")
...
# Overzicht bezoeken 2023

Onderstaande tabel geeft een overzicht van de bezoeken in 2023.
...
visits_summary %>%
  filter(year == 2023) %>%
  select(locatie = visit_location, datum = visit_start_date, opm = visit_notes, aantallen_ingevoerd = count_data, aantal_tot = number_tot) %>%
...
visits_location %>%
   filter(year >= 2021) %>%
  select(year, locatie = visit_location, n_bezoeken = n_visits, n_bezoeken_data = n_visits_data) %>%
...
jaardoel1 <- (locations_year %>%
  filter(year == 2023))$n_locaties / 19 * 100 

jaardoel2 <- (locations_year %>%
  filter(year == 2023))$n_locaties_5bezoeken / 10 * 100 
