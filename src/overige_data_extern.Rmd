...
data_mollusken <- read_excel("../data_extern/count_data/mollusken/INBODATAVR-277_divers_wbe_VL_Dump20210524.xlsx") %>%
  bind_rows(read_excel("../data_extern/count_data/mollusken/INBODATAVR-277_divers_wbe_VL_Dump20210524_bijkomend.xlsx")) %>%
...
data_roestbruine_kniptor_2020 <- read.csv2("../data_extern/count_data/roestbruine_kniptor/RoestbrKniptor_wbe_2020.csv", stringsAsFactors = FALSE) %>%
  filter(lon != "onbekend") %>%
  select(id, naam_nl, naam_lat, datum, gebied, status, x, y, aantal, gedrag, kleed, waarnemer = wnr, opmerking = bijzonderheden, uren_gevangen) %>%
  mutate(id = as.character(id),
         datum = as.Date(datum, format = "%d/%m/%Y"))

data_roestbruine_kniptor_2019 <- read_sf("../data_extern/count_data/roestbruine_kniptor", "Roestbruine_kniptor_waarnemingen_2019") %>%
    select(id, naam_nl, naam_lat, datum, gebied, status, x, y, aantal, gedrag, kleed, waarnemer = wnr, opmerking = bijzonderh) %>%
    mutate(id = as.character(id),
         datum = as.Date(datum, format = "%Y-%m-%d")) %>%
  st_drop_geometry()
...
overzicht_tellingen_inhaalslag <- data_wnm_vermiljoenkever %>%
  filter(str_detect(bijzonderheden, "#Cc")) %>%
  group_by(naam_nl) %>%
  summarise(n_tellingen = n_distinct(datum, utm1),
    n_hokken = n_distinct(utm1)) %>%
  ungroup()
```
...
```{r}
# write_vc(data_extern_burchten, "data_burchten", "../processed", strict = FALSE)
