...
planten_update_orig <- read_excel(file.path(data_path, "OpleveringMeetnetplanten_januari2025_tw.xlsx"))
...
data_waarnemingen_migratie <- data_extern_locaties %>%
  mutate(hoofdteller = ifelse(str_to_lower(waarnemer) == "vincent smeekens" & locatie == "Merksplas - Merkske - Moer (b5-27-32)", "bart hoeymans",
                            ifelse(str_to_lower(waarnemer) == "rijmenans gilbert", "gilbert rijmenans",
                                   ifelse(str_to_lower(waarnemer) == "joseph gorissen", "jos gorissen",
...
controle_bezoeken <- data_waarnemingen_migratie %>%
  filter(is.na(visit_id)) %>%
  distinct(meetnet, locatie, afstand_tot_locatie, datum, invoerdatum, waarnemer_wnm.be, code)
