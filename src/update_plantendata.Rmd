...
## Aantallen ingevoerd in meetnetten
...
aantallen_waarnemingen_mn <- data_waarnemingen_migratie %>%
  ...
  filter(!is.na(visit_id)) %>%
  select(bron, meetnet, locatie, afstand_tot_locatie, jaar, datum, hoofdteller, waarnemer_wnm.be = waarnemer, id_wnm = id, code = floroncode,  aantal, x =x_coord, y = y_coord, visit_id, opm)

aantallen_waarnemingen_extra <- data_waarnemingen_extra %>%
  filter((is.na(floroncode)) | (is.na(locatie))) %>%
  select(bron, meetnet, locatie, afstand_tot_locatie, jaar, datum, waarnemer, id_wnm = id,  code = floroncode,  aantal, x = x_coord, y = y_coord, opm)
...
controle_bezoeken <- data_waarnemingen_migratie %>%
  filter(!is.na(floroncode)) %>%
  filter(is.na(visit_id)) %>%
  select(id, meetnet, locatie, datum,  floroncode, x_coord, y_coord, waarnemer, opm)
